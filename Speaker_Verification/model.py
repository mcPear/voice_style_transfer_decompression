...
def train(path):
    ...
    # optimizer operation
    trainable_vars= tf.compat.v1.trainable_variables()                # get variable list
    ...
    with tf.compat.v1.Session() as sess:
        ...
        writer = tf.compat.v1.summary.FileWriter(os.path.join(path, "logs"), sess.graph)
        epoch = 0
...
def test(path):
    ...
    # draw graph
    enroll = placeholder(shape=[None, config.N*config.M, config.mel_size], dtype=tf.float32) # enrollment batch (time x batch x n_mel)
    verif = placeholder(shape=[None, config.N*config.M, config.mel_size], dtype=tf.float32)  # verification batch (time x batch x n_mel)
    ...
    enroll_embed = normalize(tf.reduce_mean(tf.reshape(embedded[:config.N*config.M, :], shape= [config.N, config.M, -1]), axis=1)) # tu robi średnią po wypowiedziach, czyli de facto interesuje mnie bardziej verif_embed
...
#         print(v)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        if config.tdsv:
            ...
            S = sess.run(similarity_matrix, feed_dict={enroll:random_batch(shuffle=False),# większość wypowiedzi
                                                       verif:random_batch(shuffle=False, utter_start=config.M)}) #chyba tylko ostatnia wypowiedź dla porównania ?
